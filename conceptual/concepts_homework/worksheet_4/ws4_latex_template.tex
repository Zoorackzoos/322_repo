...
% annot run program "pdflatex"
% (in directory
% "C:\Users\dunca\Desktop\code_files\CSCE\CSCE_322_AKA_programming_language_concepts_AKA_mr_t_class\322_Clion_super_folder\322_repo\conceptual\concepts_homework\worksheet_4"):
...
\begin{enumerate}
    ...
    \begin{solution}
        plug in the rightmost for the outer x
        \[
            ( \lambda x.( \lambda y.( x ( \lambda z.( x y z ) ) ) ) ) ( \lambda y. ( \lambda x. ( y z x ) ) )
            \rightarrow
            ( \lambda y.( ( \lambda y. ( \lambda x. ( y z x ) ) ) ( \lambda z.( x y z ) ) )
        \]
        plug in the rightmost for y
        \[
            ( \lambda y. ( \lambda x. ( ( \lambda z.( x y z ) ) z x ) ) )
            \rightarrow
